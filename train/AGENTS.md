...
- **Remote handoffs must finish the loop.** Remote Codex tasks should not ask for repeated confirmations; run the requested job to a final PASS/FAIL/BLOCKED result, commit useful small evidence, push it, and return the summary directly.
- **Use direct authorized GitHub network.** For GitHub `git fetch`, `git pull`, and `git push`, do not retry through a local `127.0.0.1:7897` proxy; use the already-authorized direct GitHub path or a narrow approved/escalated git command.
...
- **Use authorized direct GitHub sync.** For GitHub `git fetch`, `git pull`, and `git push` in this repo, do not spend time retrying the local `127.0.0.1:7897` proxy path after it fails. Use the already-approved direct network/escalated path for the same narrow git command instead. This does not relax credential, destructive-operation, or non-GitHub network boundaries.
